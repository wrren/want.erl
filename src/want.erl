...
-export( [atom/1, atom/2, boolean/1, string/1, binary/1, integer/1, float/1] ).
...
%%
%%  @doc Convert a float to an atom with the given precision
%%
-spec atom( float(), integer() ) -> atom().
atom( V, D ) when is_float( V )  -> want:atom( float_to_list( V, [{ decimals, D }] ) ).
